...
class View extends Object {
	 ...
	 *                        - `cache` - Can either be `true`, to enable caching using the config in
	 *                        View::$elementCache. Or an array If an array, the following keys can be used:
	 *                        - `config` - Used to store the cached element in a custom cache configuration.
	 *                        - `key` - Used to define the key used in the Cache::write(). It will be prefixed with
	 *                        `element_`
	 *                        - `plugin` - (deprecated!) Load an element from a specific plugin. This option is
	 *                        deprecated, and will be removed in CakePHP 3.0. Use `Plugin.element_name` instead.
	 *                        - `callbacks` - Set to true to fire beforeRender and afterRender helper callbacks for
