...
class HttpSocket extends CakeSocket {
	 ...
	 * @param bool|array   $base If true use default URI config, otherwise indexed array to set 'scheme', 'host',
	 *                           'port', etc.
	 *
	...
	/**
	 * This function can be thought of as a reverse to PHP5's http_build_query(). It takes a given query string and
	 * turns it into an array and supports nesting by using the php bracket syntax. So this means you can parse queries
	 * like:
	 *
	 * - ?key[subKey]=value
	...
	/**
	 * Resets the state of this HttpSocket instance to it's initial state (before Object::__construct got executed) or
	 * does the same thing partially for the request and the response property only.
	 *
	 ...
	 *
	 * @param array $request Needs to contain a 'uri' key. Should also contain a 'method' key, otherwise defaults to
	 *                       GET.
	 *
